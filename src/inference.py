...
def inference(data_dir, model_dir, output_dir, args):
    ...
    if len(args.ensemble) > 1:
        ...
        for i, model_dir in enumerate(args.ensemble):
            ...
            if "resnet18" in model_dir:
                model = load_model(model_dir, num_classes, device, "ResNet18").to(device)
            if "resnet152" in model_dir:
                model = load_model(model_dir, num_classes, device, "ResNet152").to(device)
            model.eval()
    ...
    if args.ensemble:
        info.to_csv(os.path.join(output_dir, f'ensemble_{dt_now}_output.csv'), index=False)
    else:
        info.to_csv(os.path.join(output_dir, f'{args.model}_{dt_now}_output.csv'), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize', nargs='+', type=int, default=(380, 380), help='resize size for image when you trained (default: (96, 128))')
    ...
    if args.task == 'age_regression':
        age_inference(data_dir, model_dir, output_dir, args)
    else:
