...
class CustomAugmentation:
    def __init__(self, resize, mean, std, **args):
        self.transform = transforms.Compose([
            CenterCrop((380, 380)),
            Resize(resize, Image.BILINEAR),
            ColorJitter((1, 1.5), (1, 1.5), 0.1, 0.1),
            ToTensor(),
            Normalize(mean=mean, std=std),
            RandomRotation(10)
            #AddGaussianNoise()
...
class MaskBaseDataset(Dataset):
    ...
    def __getitem__(self, index): # mask/gender/age label별 예측 task
...
class MaskSplitByProfileDataset(MaskBaseDataset):
    ...
    def __init__(self, data_dir, task, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), val_ratio=0.2):
